...
{
    private const float MINIMUM_LENGTH = 6.5f;
    ...
    private void AdjustSpreaderSize()
    {
        ...
        if (Math.Abs(GetCurrentLength() - DesiredLength) < 0.002f)
    ...
    {
        DesiredLength = Math.Max(MINIMUM_LENGTH, Math.Min(desired, MAXIMUM_LENGTH));
        dt = deltaTime;
    }
}
